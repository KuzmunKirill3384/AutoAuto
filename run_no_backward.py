#!/usr/bin/env python3
"""
Autonomous 4-wheel robot (Picamera2, 3-классовая MobileNetV3-Small).

Действия:
    • forward  — 30 % мощности
    • rotate_left / rotate_right  — 45 % мощности
"""
...
BASE_SPEED  = 30   # % duty-cycle для движения вперёд
TURN_SPEED  = 45   # % duty-cycle для разворотов

# ─── модель / инференс ────────────────────────────────────
MODEL_PATH   = Path("robot_action_cnn_2.pth")      # state-dict (3-классовый)
IMG_SIZE     = (96, 128)
...
def _set_speed(dc: int):
    """Меняет duty-cycle на всех четырёх моторах."""
    for pwm in _pwms:
...
def main():
    ...
    if not MODEL_PATH.exists():
        sys.exit(f"{MODEL_PATH} не найден – положите веса на 3 класса.")
