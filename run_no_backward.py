#!/usr/bin/env python3
"""
Autonomous 4-wheel robot (Picamera2) — 3 действия:
forward / rotate_left / rotate_right (без backward).

• Загружает MobileNetV3-Small из robot_action_cnn.pth (3-классовый state-dict)
• Каждые 0.01 с берёт кадр 640 × 480 → 128 × 96, нормализует,
  предсказывает действие и подаёт команды мотор-драйверам по GPIO.
"""
...
# ─── пины моторов (как в «рабочем» скрипте) ───────────────
FL_IN1, FL_IN2, FL_EN = 20, 21, 16
FR_IN1, FR_IN2, FR_EN = 13, 26, 19
RR_IN1, RR_IN2, RR_EN = 24, 23, 25
RL_IN1, RL_IN2, RL_EN = 7,  8,  12
PWM_FREQ_HZ, SPEED = 1000, 30  # % duty-cycle

# ─── модель и инференс ────────────────────────────────────
MODEL_PATH   = Path("robot_action_cnn_2.pth")   # именно 3-классовые веса!
IMG_SIZE     = (96, 128)                      # H × W
...
# ─── движения (полностью повторяют «рабочий» скрипт) ──────
def _all_low():
...
def main():
    ...
    parser.add_argument("--preview", action="store_true",
                        help="Показ кадра и предсказания (требуется X11)")
    args = parser.parse_args()

    if not MODEL_PATH.exists():
        sys.exit(f"{MODEL_PATH} не найден – используйте 3-классовую модель.")
    ...
    cam.start()
    capture: Callable[[], "np.ndarray"] = lambda: cam.capture_array()
